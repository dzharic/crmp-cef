...
let chat =
{
    ...
    active: true,
    timer: null,
	alpha: 0, // Включена ли прозрачность
    chatsize: 20, // Step размера окна чата
    fontstep: 10, // Step размера шрифта в чате
    prevmsg: ["", "", "", "", "", "", "", "", "", ""],
...
function enableChatInput(enable) {
    ...
    if (enable != (chat.input != null)) {
        ...
        if (enable) {
            ...
            chat.input = chatvar.append('<div><input value="" id="chat_msg" type="text" /><div class="chat-btns"> <button onclick="doInChat(writeDo)" class="btnchat">/do</button> <button onclick="doInChat(writeMe)" class="btnchat">/me</button> <button onclick="doInChat(writeTry)" class="btnchat">/try</button> <button onclick="doInChat(writeB)" class="btnchat">/b</button></div></div></ul>').children(":last");
